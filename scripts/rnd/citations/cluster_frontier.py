...
# ---------------------------------------------------------------------------
GOV_BM = "python3 -m crawlers.gov --library --deep --categories bm"   # gov.cn 部门文件 (12.7k)

REACH = [
    # --- Central: State Council + CPC ---
    (r"^(国发|国办|国函|国转)", "State Council 国务院",
     "python3 -m crawlers.gov --library --deep --categories gw", True),
    (r"^(中办|中发|中组|中宣|中编)", "CPC Central Committee",
     "(no dedicated crawler — new source)", False),
    # --- Central ministries: ALL reachable via the gov.cn 部门文件 (bm) library ---
    (r"^财", "Ministry of Finance 财政部", GOV_BM + "  (or crawlers.mof)", True),
    (r"^发改", "NDRC 发改委", GOV_BM + "  (or crawlers.ndrc)", True),
    (r"^国卫", "National Health Commission 卫健委", GOV_BM, True),
    (r"^(民发|民函)", "Ministry of Civil Affairs 民政部", GOV_BM, True),
    (r"^银发", "People's Bank of China 央行", GOV_BM, True),
    (r"^国市监", "SAMR 市场监管总局", GOV_BM + "  (or crawlers.samr)", True),
    (r"^(国科|科技)", "MOST 科技部", GOV_BM + "  (or crawlers.most)", True),
    (r"^(环发|环办|生态环境)", "MEE 生态环境部", GOV_BM + "  (or crawlers.mee)", True),
    (r"^(人社部|人社)", "MOHRSS 人社部", GOV_BM, True),
    (r"^(国土资|自然资源|国土)", "MNR 自然资源部", GOV_BM, True),
    (r"^(工信|工业和信息化)", "MIIT 工信部", GOV_BM + "  (or crawlers.miit)", True),
    (r"^(建|住建)", "MOHURD 住建部", GOV_BM, True),
    # --- Provinces / municipalities (existing crawlers) ---
    (r"^黑政", "Heilongjiang 黑龙江", "python3 -m crawlers.heilongjiang", False),
    (r"^(粤)", "Guangdong province 广东", "python3 -m crawlers.gkmlpt --site gd", False),
    (r"^(穗|广州)", "Guangzhou 广州", "(gkmlpt — verify Guangzhou coverage)", False),
    (r"^(深|深圳)", "Shenzhen 深圳", "python3 -m crawlers.gkmlpt --site sz", False),
    (r"^(苏|苏州)", "Suzhou/Jiangsu 苏州", "python3 -m crawlers.suzhou / crawlers.jiangsu", False),
    (r"^(京)", "Beijing 北京", "python3 -m crawlers.beijing", False),
    (r"^(沪)", "Shanghai 上海", "python3 -m crawlers.shanghai", False),
    (r"^(渝)", "Chongqing 重庆", "python3 -m crawlers.chongqing", False),
    (r"^(浙)", "Zhejiang 浙江", "python3 -m crawlers.zhejiang", False),
    (r"^(鄂|武)", "Hubei/Wuhan 湖北", "python3 -m crawlers.wuhan", False),
    # Guangdong cities (abbreviations incl. 中府=Zhongshan, 江府=Jiangmen, 云府=Yunfu, 河府=Heyuan)
    (r"^(珠|佛|莞|惠|中山|中府|江门|江府|江自|肇|茂|湛|汕|潮|揭|云浮|云府|阳|韶|梅|清远|河源|河府)",
     "Other Guangdong cities", "python3 -m crawlers.gkmlpt --site <city>", False),
]
