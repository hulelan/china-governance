...
async def _search_seg_bm25(db, match, date_clause, date_params, site_clause,
                           site_params, next_idx, search_pattern, per_page, offset):
...
async def search_documents(db, query: str, page: int = 1, per_page: int = 50,
    ...
    Ranking chain, best available first:
      1. **BM25 relevance** (`doc_search_seg`, word-segmented FTS5 over jieba
         tokens) — bm25() with a title boost + recency tiebreak. The relevance path.
      2. **Trigram substring** (`doc_search`) — indexed substring match, date-ordered;
         used when the segmented index is absent or BM25 finds nothing (recall net).
      3. **LIKE scan** — final fallback for very short queries / no indexes.
    exclude_sites / include_sites: optional lists of site_keys for the source-TYPE
    ontology filter (e.g. exclude every news site_key) — applied in ALL paths.
    ...
    # args relative to the scalar $1 FTS-match placeholder. Built before the
    # ranked paths so it advances next_idx and applies to all of them.
    site_clause = ""
    site_params = []
    if include_sites:
        ...
        try:
            ...
            if seg_match:
                rows, total = await _search_seg_bm25(
                    db, seg_match, date_clause, date_params, site_clause,
                    site_params, next_idx, search_pattern, per_page, offset)
        except Exception:
